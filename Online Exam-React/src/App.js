...
function Nav(props) {
	...
	function handleClick(num) {
		...
		// 	document.getElementById("q" + (num - 1)).style.backgroundColor = "orange";
	...
	return (
		<div className="nav" >
			{qno.map((num, i) => <div><button key={i} id={"q" + i} style={{ height: '50px', width: '50px', fontSize: '20px', margin: '3.5px' }} onClick={() => handleClick(num)}>{num}</button><br /></div>)}
		</div>
	)
}
